...
Many people got into CS by playing video games and wanting to make more. But, even if you didn't, learning how to make something simple and fun is quite rewarding, not to mention nice to show off to other people.
...
* now git clone the starter repo [repo](https://github.com/ranchncarrots/sumOfCodeGameStarterCode).  <br>

```js
$ git clone git@github.com:ranchncarrots/sumOfCodeGameStarterCode.git
```

## Workshop

First, we will import the following. We'll be using these libraries as we go along.  <br>
...
Here we are declaring our width, height, and size variables. We are then use these values in a built-in pygame function to create our screen.  <br>

Every game has what is called a game loop. It's an infinite loop goes on for the game's lifespan and is where we put all of our game logic.
...
Here we've done the following. We have set our tick rate to 60 fps using the built-in pygame function. We then fill our screen with black(this will become clearer later) and update the display. However, at this point, you should be unable to see a window on your screen. That is because we are yet to handle the event queue:
...
In pygame, groups are used to 'group' similar characters to operate on them all at once. To define a group before you game loop, put:
...
Once our `mainCharacter` is in a group, we can also draw it out. Insert ``main_sprite.draw(screen)`` inside of your loop. You should now see an image in the center of your screen. <br>

At this point your coulde should look like:
...
Now you should be slowing down after you release the key. But we can go off screen. To fix that we add a bounce at the boundaries of the screen:
...
`pygame.sprite.spritecollideany(main, badSprites)` returns either None or a sprite name that collided. If it's not `None`, we know there is a collision and if it has been outside of the grace period of the last hit, we go ahead and call update to deduct a life. 
...
In line 3, we are declaring the lines we want to use. In line 2, we declare the rendering we wish to use. Then, in line 3, we can just blit it onto the screen.
...
* [What we still haven’t learned from Gamergate](https://www.vox.com/culture/2020/1/20/20808875/gamergate-lessons-cultural-impact-changes-harassment-laws) - the rise of gaming and gaming subcultures is a really cool way computer science has hit the world today, however, it also has had many issues with gender diversity. Here's a hindsight look from a large scale issue from 2014 and it's long term impact.

## Open Source - Permissive vs Copyleft Licenses

There are two _broad_ categories in open-source licenses that most fall under called Permissive and Copyleft. 

Permissive licenses include ones like `MIT` or `ApacheV2`. These basically say do _whatever_ with the code, and if you add the same code over, you often have to include the license. These are popular for small projects when people don't care too much and want their open-source software to be good enough for commercial use. The workshop repos use this, for example.

Copyleft licenses are stronger, such as `GPLv3`, where they want future software development to also have the same freedoms the software was available under. Basically, that means that your work has to also end up open source. This is often used by large projects like `Linux` or by people who want to increase the spread of Open Source. However, this type of license makes it hard for commercial use since most of the software is proprietary (i.e., not open source). The repo for the workshops site itself uses GPLv3 as the template uses GPLv3.
