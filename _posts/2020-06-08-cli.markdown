...
Lets use this as a chance to show off the red-green-refactor workflow (more details [here](/testing101)) with `pytest`.
...
Now let's create a checkbox-like system that lets the user move up and down through the files. We'll use `PyInquirer` to ask. The format for checkbox questions is similar to this [example](https://github.com/CITGuru/PyInquirer/blob/master/examples/checkbox.py) from their documentation.
...
Note: This might lead to a `pytest` warning complaining about some deprecation but don't worry about it. It's a specific issue with `PyInquirer`.
...
* [towards data science - The ethical automation toolkit](https://towardsdatascience.com/the-ethical-automation-toolkit-f1fd4281534e)
