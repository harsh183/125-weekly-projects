...
This is great, but not everyone takes the same exact courses. What we need is allowing people to add their own and remove existing courses. For this we just have to create functions that can add and remove to form elements. We'll use js map/filter to our advantage. 
