...
Now that our code functions are set up nicely, let's create a loop for actually creating files. To create folders we use the built in `os` module's `mkdirs` to set it up. Let's create a folder per week. And to download the each file we're using `with open` with the mode `wb` to set up our downloads.
